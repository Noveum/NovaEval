...
class TestNoveumPlatformIntegration:
    ...
    def test_02_ingest_trace(self, noveum_client, sample_traces, test_context):
        ...
        for wait_time in wait_times:
            ...
            try:
                ...
                if get_response["success"] and get_response["data"]["trace_id"] == trace_id:
                    print(f"Successfully retrieved trace after {wait_time} seconds")
                    trace_retrieved = True
                    break
                else:
                    print(f"Trace retrieval failed after {wait_time} seconds: {get_response}")
            except Exception as e:
                print(f"Exception during trace retrieval after {wait_time} seconds: {e}")
        
        # Fail the test if we couldn't retrieve the trace after all attempts
        if not trace_retrieved:
            pytest.fail("Failed to retrieve trace after waiting 35 seconds total (5+10+20). Single trace ingestion may not be working properly.")
    ...
    def test_03_ingest_traces(self, noveum_client, sample_traces, test_context):
            ...
            try:
                ...
                if query_response["success"] and len(query_response["traces"]) > 0:
                    ...
                    if batch_trace_id:
                        ...
                        if get_response["success"] and get_response["data"]["trace_id"] == batch_trace_id:
                            print(f"Successfully retrieved batch trace after {wait_time} seconds")
                            trace_retrieved = True
                            break
                        else:
                            print(f"Batch trace retrieval failed after {wait_time} seconds: {get_response}")
                    else:
                        print(f"No batch traces found after {wait_time} seconds")
                else:
                    print(f"Trace query failed after {wait_time} seconds: {query_response}")
        ...
        if not trace_retrieved:
            pytest.fail("Failed to retrieve a trace from the batch after waiting 35 seconds total (5+10+20). Batch trace ingestion may not be working properly.")
    ...
    def test_21_complete_flow(self, noveum_client, sample_traces, integration_dataset_name):
