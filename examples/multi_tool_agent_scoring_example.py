...
def main():
    ...
    agent_data = AgentData(
        ...
        parameters_passed={"operation": "add", "a": 20, "b": 22, "key": "calculation_result", "value": "42", "location": "New York"},
        tool_call_results=[
            ToolResult(
                call_id="call_001",
                result=42,
    ...
    if isinstance(tool_relevancy, list):
        ...
        print(f"   Average Relevancy: {sum(relevancy_scores) / len(relevancy_scores):.2f}\n")
    else:
    ...
    if isinstance(param_correctness, list):
        ...
        for i, score_obj in enumerate(param_correctness, 1):
            call = agent_data.tool_calls[i-1]
            result = next((r for r in (agent_data.tool_call_results or []) if r.call_id == call.call_id), None)
            success_status = "✓" if result and result.success else "✗"
            print(f"   Tool Call {i} ({call.tool_name}) {success_status}: {score_obj.score}")
            print(f"      Reasoning: {score_obj.reasoning}")
            param_scores.append(score_obj.score)
        print(f"   Average Parameter Correctness: {sum(param_scores) / len(param_scores):.2f}\n")
    else:
    ...
    # Summary
    print("=== Summary ===")
    if (isinstance(tool_relevancy, list) and isinstance(tool_correctness, list) and 
        isinstance(param_correctness, list) and relevancy_scores and correctness_scores and param_scores):
        ...
        print(f"Most Relevant Tool: {agent_data.tool_calls[max_relevancy_idx].tool_name} ({max(relevancy_scores)})")
        print(f"Most Correct Tool: {agent_data.tool_calls[max_correctness_idx].tool_name} ({max(correctness_scores)})")
        print(f"Best Parameters: {agent_data.tool_calls[max_param_idx].tool_name} ({max(param_scores)})")
        print(f"Least Relevant Tool: {agent_data.tool_calls[min_relevancy_idx].tool_name} ({min(relevancy_scores)})")
