...
def test_prompt_formatting_safety():
    ...
    try:
        formatted = TOOL_RELEVANCY_PROMPT.format(
            tools_available="[sample tools]",
            tool_calls="[sample calls]"
        )
        assert len(formatted) >= len(TOOL_RELEVANCY_PROMPT) - 50  # Allow for placeholder replacement
    except KeyError:
    ...
    try:
        formatted = TOOL_CORRECTNESS_PROMPT.format(
            expected_tool_call="sample expected call",
            tool_calls="[sample calls]"
        )
...
def test_prompt_lengths():
    ...
    for prompt in prompts:
        ...
        assert 500 < len(prompt) < 5000, f"Prompt length {len(prompt)} is outside reasonable range" 