...
from novaeval.config.schema import EvaluationJobConfig, ModelProvider, DatasetType, ScorerType
...
class DatasetFactory:
    ...
    def create_dataset(dataset_config) -> BaseDataset:
        ...
        elif dataset_type in [DatasetType.CUSTOM, DatasetType.JSON, DatasetType.CSV, DatasetType.JSONL]:
...
class JobRunner:
    ...
    async def _evaluate_model_dataset(
        ...
        evaluator = Evaluator(
            model=model,
            dataset=dataset,
            scorers=[scorer for _, scorer in scorers]
        )
    ...
    def _compile_results(self, evaluation_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "models_evaluated": len(set(r['model']['name'] for r in evaluation_results)),
            "datasets_used": len(set(r['dataset']['name'] for r in evaluation_results)),
    ...
    def _check_ci_requirements(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        ci_passed = len(failed_evaluations) == 0 if ci_config.fail_on_threshold else True
        ...
        return {
            ...
            "recommendation": "Deploy" if ci_passed else "Do not deploy - evaluations below threshold"
        }
...
def main():
    ...
    try:
        ...
        if result["status"] == "completed":
            ...
            print(f"CI Status: {'✅ PASSED' if result['ci_status']['passed'] else '❌ FAILED'}")
