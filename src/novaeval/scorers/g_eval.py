...
class GEvalCriteria(BaseModel):
    ...
    score_range: tuple[int, int] = Field(default=(1, 5), description="Score range (min, max)")
...
class GEvalScorer(BaseScorer):
    ...
    def _build_prompt(self, input_text: str, output_text: str, context: Optional[str] = None) -> str:
    ...
    async def evaluate(
        ...
        **kwargs
        ...
        for i in range(self.num_iterations):
            score, reasoning = await self._evaluate_single(input_text, output_text, context)
            scores.append(score)
