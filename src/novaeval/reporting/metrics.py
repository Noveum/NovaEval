...
class MetricsCalculator:
    ...
    def calculate_aggregated_metrics(
        ...
        if filter_provider:
            filtered_metrics = [m for m in filtered_metrics if m.provider == filter_provider]
        if filter_model:
    ...
    def calculate_provider_comparison(self) -> Dict[str, AggregatedMetrics]:
        ...
        """
        providers = set(m.provider for m in self.request_metrics if m.provider)
        return {
    ...
    def calculate_model_comparison(self) -> Dict[str, AggregatedMetrics]:
        ...
        """
        models = set(m.model for m in self.request_metrics if m.model)
        return {
    ...
    def _calculate_cost_metrics(self, metrics: List[RequestMetrics]) -> Dict[str, Any]:
        ...
        total_tokens = sum(m.tokens_total for m in metrics if m.tokens_total is not None)
    ...
    def _calculate_error_breakdown(self, failed_metrics: List[RequestMetrics]) -> Dict[str, int]:
    ...
    def export_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "unique_providers": len(set(m.provider for m in self.request_metrics if m.provider)),
                "unique_models": len(set(m.model for m in self.request_metrics if m.model)),
