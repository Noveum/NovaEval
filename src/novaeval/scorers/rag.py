...
class AnswerRelevancyScorer(BaseScorer):
    ...
    async def evaluate(
        ...
        **kwargs
        ...
        try:
            generated_questions_response = await self.model.generate(question_generation_prompt)
            ...
            for gen_embedding in generated_embeddings:
                similarity = np.dot(original_embedding[0], gen_embedding) / (
                    np.linalg.norm(original_embedding[0]) * np.linalg.norm(gen_embedding)
                )
            ...
            return ScoreResult(
                score=0.0,
                passed=False,
                reasoning=f"Answer relevancy evaluation failed: {str(e)}",
    ...
    async def evaluate(
        ...
        **kwargs
        ...
        try:
            ...
            for claim, verification in verification_results:
            ...
            return ScoreResult(
                score=0.0,
                passed=False,
                reasoning=f"Faithfulness evaluation failed: {str(e)}",
                metadata={"error": str(e)}
            )
        ...
        **kwargs
        ...
        try:
            ...
            return ScoreResult(
                score=0.0,
                passed=False,
                reasoning=f"Contextual precision evaluation failed: {str(e)}",
                metadata={"error": str(e)}
            )
        ...
        **kwargs
        ...
        try:
            ...
            return ScoreResult(
                score=0.0,
                passed=False,
                reasoning=f"Contextual recall evaluation failed: {str(e)}",
                metadata={"error": str(e)}
            )
...
class RAGASScorer(BaseScorer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.contextual_precision_scorer = ContextualPrecisionScorer(model, threshold=0.7)
        ...
        **kwargs
        ...
        try:
            ...
            results = await asyncio.gather(
                self.answer_relevancy_scorer.evaluate(input_text, output_text, expected_output, context),
                self.faithfulness_scorer.evaluate(input_text, output_text, expected_output, context),
                self.contextual_precision_scorer.evaluate(input_text, output_text, expected_output, context),
                self.contextual_recall_scorer.evaluate(input_text, output_text, expected_output, context),
            ...
            metric_names = ["answer_relevancy", "faithfulness", "contextual_precision", "contextual_recall"]
            
            for i, (metric_name, result) in enumerate(zip(metric_names, results)):
                if isinstance(result, Exception):
                    scores[metric_name] = 0.0
                    reasonings[metric_name] = f"Error: {str(result)}"
                else:
